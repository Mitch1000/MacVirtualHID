...
class DriverLoadingViewModel: NSObject {
    ...
    private let dextIdentifier: String = "com.mogindustries.user-client.app.OpenVirtualHIDDriver"

    public var dextLoadingState: String {
        switch state {
        case .unloaded:
            return "OpenVirtualHIDDriver isn't loaded."
        case .activating:
            return "Activating OpenVirtualHIDDriver, please wait."
        case .needsApproval:
            return "Please follow the prompt to approve OpenVirtualHIDDriver."
        case .activated:
            return "OpenVirtualHIDDriver has been activated and is ready to use."
        case .activationError:
            return "OpenVirtualHIDDriver has experienced an error during activation.\nPlease check the logs to find the error."
        }
    }
}
