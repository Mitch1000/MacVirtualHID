...
unsigned char gamepad_report_descriptor[] = {
  ...
  0x95, 0x3F,   //       (GLOBAL) REPORT_COUNT       0x3F (63) Number of fields <-- Redundant: REPORT_COUNT is already 63
  0xB1, 0x02,   //       (MAIN)   FEATURE            0x00000002 (63 fields x 8 bits) 0=Data 1=Variable 0=Absolute 0=NoWrap 0=Linear 0=PrefState 0=NoNull 0=NonVolatile 0=Bitmap
  0xC0,         //  (MAIN)   END_COLLECTION     Application  <-- Warning: Physical units are still in effect PHYSICAL(MIN=0,MAX=315) UNIT(0x00000000,EXP=0)        // END_COLLECTION
};

} // namespace

struct OpenVirtualHIDDevice_IVars {
    OpenVirtualHIDClient* provider;
  bool ready;
  uint8_t lastLedState;
};

bool OpenVirtualHIDDevice::init() {
  Log(" init");
  ...
  ivars = IONewZero(OpenVirtualHIDDevice_IVars, 1);
  if (ivars == nullptr) {
    return false;
  }
...
void OpenVirtualHIDDevice::free() {
  Log(" free");

  IOSafeDeleteNULL(ivars, OpenVirtualHIDDevice_IVars, 1);

  super::free();
}

bool OpenVirtualHIDDevice::handleStart(IOService* provider) {
  Log(" handleStart");

    ivars->provider = OSDynamicCast(OpenVirtualHIDClient, provider);
  if (!ivars->provider) {
    Log("Provider is not correct client");
    return false;
  }

  if (!super::handleStart(provider)) {
    Log(" super::handleStart failed");
    return false;
  }
...
kern_return_t IMPL(OpenVirtualHIDDevice, Stop) {
  Log(" Stop");
...
OSDictionary* OpenVirtualHIDDevice::newDeviceDescription(void) {
  Log(" newDeviceDescription");

  auto dictionary = OSDictionary::withCapacity(12);
  if (!dictionary) {
    Log(" OSDictionary::withCapacity failed");
    return nullptr;
  }
  ...
  uint32_t keyboardProductId = 0x05c4;
  if (ivars->provider) {
...
OSData* OpenVirtualHIDDevice::newReportDescriptor(void) {
  Log(" newReportDescriptor");

  return OSData::withBytes(gamepad_report_descriptor, sizeof(gamepad_report_descriptor));
}

kern_return_t OpenVirtualHIDDevice::setReport(IOMemoryDescriptor* report,
  ...
  if (kr != kIOReturnSuccess) {
    Log(" setReport Map error: 0x%x", kr);
    return kr;
  }
  ...
  kr = IOBufferMemoryDescriptorUtility::createWithBytes(&ledReport,
                                                        ...
                                                        &memory);
  if (kr != kIOReturnSuccess) {
    Log(" setReport createWithBytes error: 0x%x", kr);
    return kr;
  }
...
kern_return_t IMPL(OpenVirtualHIDDevice, postReport) {
  if (!report) {
    Log("Post Report: Report does not exist");
    return kIOReturnBadArgument;
  }
  ...
  if (kr != kIOReturnSuccess) {
    Log("Post Report: Could not get report length");
    return kr;
  }
  
  Log("Post Report: Attempting call to Handle Report %i", int(reportLength));
...
kern_return_t IMPL(OpenVirtualHIDDevice, reset) {
  Log(" reset");

  return kIOReturnSuccess;
}

bool IMPL(OpenVirtualHIDDevice, getReady) {
  return ivars->ready;
}
