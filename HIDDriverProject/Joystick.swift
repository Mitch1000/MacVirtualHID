...
//    var hidVirtualDevice: HIDVirtualDevice?
    ...
    let VIRTGAMEPAD_MAX_DEVICE_HID_INPUT_VALUES = 8
    let VIRTGAMEPAD_ARG_COUNT = 7         // device arguments count
    let virtualgamepadInputCount: Int;
...
//    let SERVICE_NAME = "AppleUserHIDDevice"
    ...
    init(id: Int) {
        self.virtualgamepadInputCount = VIRTGAMEPAD_ARG_COUNT + VIRTGAMEPAD_MAX_DEVICE_HID_INPUT_VALUES
        self.virtgamepadInput = [UInt64](repeating: 0, count: virtualgamepadInputCount)
        self.id = id
    }
...
//        let gamepadReportDescriptor: [UInt8] = [0x05, 0x01]
        ...
        let deviceDescriptorString:String = """
        6709325968011578977737313578313176731930031914151964\
        1241528218698414758466682765338713442932187586116480062797787024513302786166059841545697100515623747776
        """

        let deviceHidInputCount = 1...VIRTGAMEPAD_MAX_DEVICE_HID_INPUT_VALUES
        let descriptorStringSectionLength = deviceDescriptorString.count / VIRTGAMEPAD_MAX_DEVICE_HID_INPUT_VALUES
        
        for i in deviceHidInputCount {
            let offset = (i - 1) * descriptorStringSectionLength

            let startIndex = deviceDescriptorString.index(deviceDescriptorString.startIndex, offsetBy: offset)
            let endIndex = deviceDescriptorString.index(startIndex, offsetBy: descriptorStringSectionLength)
            let descriptorSectionString = deviceDescriptorString[startIndex..<endIndex]

            guard let descriptorSection = UInt64(descriptorSectionString) else {
                print("DescriptorSection Int Conversion error")
                virtgamepadInput[6 + i] = 0
                continue
            }
            print("DescriptorSection", descriptorSection)
            virtgamepadInput[6 + i] = descriptorSection
        }
        print(virtgamepadInput)
    ...
    func callCreateMethod() async throws {
        ...
        let ret = IOConnectCallScalarMethod(
            ...
            gamepadInput,
            UInt32(virtualgamepadInputCount),
            nil,
            outputCnt
        )
